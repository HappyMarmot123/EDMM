...
- UI 정책: desktop/mobile player 분기, `pop`/`edm`/`recent` 음악 뷰, 공용 Radix tooltip surface, 공용 route feedback shell 사용
- 모바일 시스템 미디어 UI: Media Session metadata/action/position/playbackState를 `useMediaSession`에서 통합 관리
...
- `/search?view=pop|edm|recent`
  - `pop`: 기본 카탈로그 뷰, Cloudinary `pop` category 조회
  - `edm`: Cloudinary `edm` category 조회
  - `recent`: Dexie 최근 재생 ID 기반 목록
  - 그 외 값은 `pop`으로 정규화
...
- `appProviders.tsx`가 전역 의존성을 조립:
  ...
  - `ToggleProvider`
  - `widgets/audioPlayer`
- `shared/components/routeFeedbackShell.tsx`가 loading/error/not-found 라우트 화면의 공통 shell을 제공
...
- `search/page.tsx`는 `widgets/musicShell/musicView.ts`의 route adapter로 `view`를 해석하고, 클라이언트 뷰 조립 책임을 `searchPageClient.tsx`에 위임
- `/track/[id]`는 canonical URL 정책상 상세 라우트에서 검색 흐름으로 정규화
- `loading.tsx`, `search/loading.tsx`, `track/[id]/loading.tsx`, `error.tsx`, `not-found.tsx`는 `RouteFeedbackShell`로 공통 레이아웃을 공유
...
- `features/cloudinary`: 트랙 조회 훅, Cloudinary category 조회, video/image artwork 병합
...
- `widgets/musicShell/musicView.ts`: `pop | edm | recent` 뷰 vocabulary와 라우트 파싱의 단일 기준
...
- `shared/components/routeFeedbackShell.tsx`: 라우트 loading/error/not-found 화면의 공통 feedback shell
...
2. `musicView.ts`가 `view` 값을 `pop | edm | recent`로 검증하고, 기본값은 `pop`으로 정규화
3. `searchPageClient.tsx`가 초기 뷰와 초기 트랙 ID를 전달
4. `AudioPlayerShell`이 `useAudioPlayer().playTrack`을 `SearchView`에 전달
5. `views/search/index.tsx`에서 `MusicShell` 렌더
6. `MusicShell`은 아래 상태를 결합:
   - `selectedTrackId`
   - `selectionSource` (`initial`/`visible`/`none`)
   - `visibleTracks`
   - `currentTrackId`
7. `musicShellHeader`에서 `pop`, `edm`, `recent` 뷰와 검색어를 제어하고 `pop`/`edm` 전체 카운트를 badge로 표시
8. `MusicShell`은 `pop`/`edm` 카탈로그를 `resourceType: "all"` + `category` 조합으로 조회하고, `recent`는 Dexie 최근 재생 캐시로 복원
9. `musicTrackList`는 Virtuoso 기반 결과 목록 렌더와 사용자 선택 이벤트 전달을 담당하며, 부모 handler 안정화와 `memo`로 불필요한 재렌더를 줄임
10. `trackDetailAside`는 선택된 ID 기준의 상세 렌더를 담당
11. 모바일에서는 `TrackDetailAside`를 숨기고, row select는 즉시 재생까지 수행
12. 데스크톱 row select는 상세 선택만 수행하고, play 버튼이 재생 intent를 발생시킴
...
participant Tracks as "useCloudinaryTracks(category)/useRecentPlays"
...
Shell->>Tracks: fetch pop/edm catalog, active query, recent ids
...
- 명시적 `view`가 없고 초기 track이 `pop` 또는 `edm` 카탈로그에서 확인되면, MusicShell은 해당 카탈로그 뷰로 자동 정렬
- 사용자가 수동으로 뷰를 바꾼 이후에는 자동 카탈로그 정렬을 중단해 사용자 선택을 보존
...
- player track zone 선택 이벤트는 track ID가 속한 `pop`/`edm` 카탈로그를 찾은 뒤 필요한 경우 뷰 전환과 목록 스크롤 요청을 분리해 처리
...
- Provider 내부 로직은 훅으로 분리:
  ...
  - `useMediaSession`: OS 미디어 세션 메타데이터, action handler, position state, playback state
...
- Media Session은 `play`, `pause`, `previoustrack`, `nexttrack`, `seekto`를 Provider 액션으로 연결하고, `currentTime`/`duration`을 `setPositionState`에 반영
- 모바일 백그라운드/시스템 미디어 UI의 진행 상태 안정성을 위해 `isPlaying` 변화는 `mediaSession.playbackState`(`playing`/`paused`/cleanup 시 `none`)로 명시 반영
...
- 통합 라우트는 `resourceType`, `filterPlayable`, `category`, `v` 쿼리를 해석하고 Cloudinary client option으로 전달
- `category=pop|edm`은 Cloudinary 기본 폴더 아래 category subfolder로 scope를 좁히는 방식

### 7.2 어댑터 및 API Client

- `features/cloudinary/hooks/useCloudinaryTracks.ts`가 쿼리 타입(`all`, `video`, `image`)에 따라 결과 결합 및 변환 수행
- `resourceType: "all"`은 클라이언트에서 `/api/cloudinary/tracks?resourceType=all` 단일 요청으로 조회하고, 응답을 video/image로 분리한 뒤 비디오 트랙에 이미지 트랙을 artwork fallback으로 병합
- `filterPlayable`은 `resourceType: "all"` 응답 분리 후 video 트랙에만 적용하며, image 트랙은 artwork 병합 후보로 유지
- TanStack Query key는 cache version, 검색어, resourceType, filterPlayable, category를 포함해 카탈로그별 캐시 충돌을 피함
...
- mobile catalog:
  - `pop`/`edm`/`recent` view model은 데스크톱과 공유
  - `TrackDetailAside`는 모바일에서 숨김
  - row select는 재생 intent까지 즉시 전달
- mobile fullscreen:
...
- route feedback 화면은 `shared/components/routeFeedbackShell.tsx`를 공통 shell로 사용하고, 각 라우트는 copy/action만 유지

### 9.2 기능 계층

- 검색/플레이 목록: 상태 기반 렌더 fallback
- 카탈로그 조회 실패: stale catalog가 있으면 stale 목록을 유지하고 feedback notice를 노출
- 최근 재생/트랙 캐시 조회 실패: IndexedDB unavailable 이벤트를 Sentry taxonomy로 기록하고 빈 기본값으로 UI를 유지
